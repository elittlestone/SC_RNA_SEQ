...
**Before we begin to annotate our clusters, we have to decide on the resolution of the leiden algorithm. The higher the resolution, the more distinct clusters we get, and vice versa. This means that if we pick too low of a resolution, we can only look at large/general cell-types (B-cells, T-cells, etc), versus if we pick too large a resolution, we risk segregating identical cell-types into different clusters. I plotted the different resolutions below and decided to choose 0.5**
