...
pixi add snakemake geofetch sra-tools ncbi-datasets-cli cellranger
```
...
**Sratools is a suite of command-line tools that allows for the programmatic downloading of GEO data. This avoids having to download through the web and promotes reproducibility as well as error handling. Sratools comes with multiple programs, here we use the *fastq-dump* command. Running fastq-dump -h' after downloading sratools through pixi gives helpful instructions on how to use the tool as well as the variety of different flags/parameters that can be passed to this program. We use fastq-dump in a snakemake script to download our rna-seq reads and convert them to fastq format.**


**Raw paired-read FASTQ files from this scRNA-seq experiment are available online under the GEO accession ID GSE. To obtain direct links to each of the 12 samples in bulk, we query this accession ID in SRA explorer, select the relevant samples, then download the metadata to a csv, shown below.**
